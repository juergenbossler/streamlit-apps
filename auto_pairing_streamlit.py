...
st.set_page_config(page_title="AUTO Pairing Simulation", page_icon='GMD-Digital_icon.png', layout="wide", initial_sidebar_state='collapsed')
...
with row1_3:
    st.title('AUTO Pairing Simulation 4008')
...
ralu_choice = st.sidebar.radio('ball size calculation - utilization of radial play', ('use full tolerance', 'aim tolerance center'), index=0)
...
for i in range(buffersize):
    # Zufallsnummer aus Liste Losgröße (z.B. n=750)
...
while len(values_AR) > 0:
    ...
    #print('buffer_list_AR geleert ')
    #print(buffer_list_AR)
    #print('buffer_list_IR geleert ')
    #print(buffer_list_IR)
    #print('Länge values_AR davor: ' + str(len(values_AR)))

    for i in range(buffersize):
        if len(values_AR) > 0:
            # Liste Los Größe
            picknumber_list_AR = list(range(len(values_AR)))
            #print(picknumber_list_AR)
            # Zufallsnummer aus Liste Losgröße (z.B. n=750)
            picknumber_AR = random.choice(picknumber_list_AR)
            # Auswahl Teil mit Zufallsmummer als Index
            picked_piece_AR = values_AR[picknumber_AR]
            # Liste (Gruppe) ausgeählter Teile
            buffer_list_AR.append(picked_piece_AR)
            # Ausgewähltes Teil aus Liste entfernen
            values_AR.pop(picknumber_AR)

            # Liste Los Größe
            picknumber_list_IR = list(range(len(values_IR)))
            #print(picknumber_list_IR)
            # Zufallsnummer aus Liste Losgröße (z.B. n=750)
            picknumber_IR = random.choice(picknumber_list_IR)
            # Auswahl Teil mit Zufallsmummer als Index
            picked_piece_IR = values_IR[picknumber_IR]
            ...
            buffer_list_IR.append(picked_piece_IR)
            # Ausgewähltes Teil aus Liste entfernen
            values_IR.pop(picknumber_IR)
        else:
            continue

    #print('buffer_list_AR gefüllt')
    #print(buffer_list_AR)
    #print('buffer_list_IR gefüllt')
    #print(buffer_list_IR)
    #print('Länge values_AR danach: ' + str(len(values_AR)))
...
with col4:
    ...
    with st.expander('AUTO Pairing'):
       ...
       st.subheader("Ball size sequence plots")
       ...
       plt.plot(diff_buff)
       plt.title("Sequence of ball sizes for AUTO pairing")
